            ...
            @if(Route::currentRouteName() == 'posts.edit')
                <form action="{{ route('posts.update', ['post'=>$post->id]) }}" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <input type="text" class="form-control" placeholder="输入文章标题" name="title" value="{{ $post->title or old('title') }}">
                        @csrf
                        {{ method_field('PUT') }}
                    </div>
                    <div class="form-group">
                    ...
                    <button type="submit" class="btn btn-primary btn-block mb-5">更新</button>
                </form>
            @else
